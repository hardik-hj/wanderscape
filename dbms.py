...
def insert_preset():
    ...
    conn.commit()
    cur.execute("""INSERT INTO city VALUES('C01',"Goa"),('C02',"Kovalam (Kerala)"),('C03',"Andaman and Nicobar"),('C04',"Manali - H.P."),
...
def get_destination_info(city_name):
    ...
    info = {
        ...
        f'Our choice of Sightseeing Places in {city}': data[2] + ':  ₹' + str(price[1]),
...
def search_cities(query):
    try:
        ...
        # Execute a query to fetch cities matching the query
        cursor.execute("SELECT city FROM city WHERE city LIKE ?", ('%' + query + '%',))
        cities = cursor.fetchall()

        # Close the connection
        conn.close()

        # Return the cities in a list format
...
def get_sightseeing_by_budget(budget_value):
    """
    Fetch sightseeing places based on the budget range.
    """
    # Calculate budget range
    min_budget = 0  # This is the minimum budget
    max_budget = budget_value  # Add 1000 to define an upper range for flexibility
    ...
    try:
        ...
        # SQL query to fetch sightseeing places within the budget range
        query = '''
        ...
        # Execute query with the budget range
        ...
        places = []
        for row in results:
            places.append({'sightseeing': row[0], 'city': row[1]})
